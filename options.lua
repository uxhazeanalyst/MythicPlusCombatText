...
for k,v in pairs(Options.defaults) do
    if type(v)=="table" then
        ...
        for key,val in pairs(v) do
            if MyCombatTextDB and MyCombatTextDB[k] and MyCombatTextDB[k][key] ~= nil then
                Options.settings[k][key] = MyCombatTextDB[k][key]
            else
                Options.settings[k][key] = val
            end
        end
    else
        if MyCombatTextDB and MyCombatTextDB[k] ~= nil then
            Options.settings[k] = MyCombatTextDB[k]
        else
